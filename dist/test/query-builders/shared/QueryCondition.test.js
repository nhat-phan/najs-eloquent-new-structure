...
describe('QueryCondition', function () {
    ...
    describe('constructor()', function () {
        ...
        it('init "isSubQuery" = false & queries = []', function () {
            const query = Reflect.construct(QueryCondition_1.QueryCondition, []);
    ...
    describe('static .create()', function () {
        it('creates new QueryCondition instance then calls .buildQuery() then returns the instance', function () {
            const spy = Sinon.spy(QueryCondition_1.QueryCondition.prototype, 'buildQuery');
            ...
            expect(query).toBeInstanceOf(QueryCondition_1.QueryCondition);
            spy.restore();
        });
    });
    ...
    describe('.buildQuery()', function () {
        it('build sub-query with provided 1 param', function () {
            const query = QueryCondition_1.QueryCondition.create(convention, 'and', function (subQuery) {
        ...
        it('build query with provided operator if passed 3 params', function () {
            const query = QueryCondition_1.QueryCondition.create(convention, 'or', 'a', 'not-in', 'b');
            expect(query.toObject()).toEqual({
                bool: 'or',
                operator: 'not-in',
                field: 'a',
