    ...
    for (const method of functions) {
        describe(`.${method}()`, function () {
            it(`calls .newQuery() then calls .${method}() with original arguments`, function () {
                ...
                const stub = Sinon.stub(model, 'newQuery');
                stub.returns(query);
                expect(QueryPublicApi_1.QueryPublicApi[method].apply(model)).toEqual('anything');
                expect(spy.calledWith()).toBe(true);
                spy.resetHistory();
                expect(QueryPublicApi_1.QueryPublicApi[method].apply(model, [1])).toEqual('anything');
                expect(spy.calledWith(1)).toBe(true);
                spy.resetHistory();
                expect(QueryPublicApi_1.QueryPublicApi[method].apply(model, [1, 2])).toEqual('anything');
                expect(spy.calledWith(1, 2)).toBe(true);
                spy.resetHistory();
                expect(QueryPublicApi_1.QueryPublicApi[method].apply(model, [1, 2, 3])).toEqual('anything');
