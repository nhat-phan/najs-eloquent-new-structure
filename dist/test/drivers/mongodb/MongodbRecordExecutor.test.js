...
describe('MongodbRecordExecutor', function () {
    ...
    describe('.restore()', function () {
        ...
        it('should work with expected log', async function () {
            ...
            await makeExecutor(model, new Record_1.Record({ id: id })).create();
            await makeExecutor(model, new Record_1.Record({ id: id })).softDelete();
            const result = await makeExecutor(model, new Record_1.Record({ id: id })).restore();
            expect_query_log({
                // tslint:disable-next-line
