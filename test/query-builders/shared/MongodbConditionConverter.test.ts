import 'jest'
import { MongodbConditionConverter } from '../../../lib/query-builders/shared/MongodbConditionConverter'
...
describe('MongodbConditionConverter', function() {
  ...
  describe('protected convertSimpleCondition()', function() {
    ...
    const converter = new MongodbConditionConverter([])
    for (const name in equalsOperatorDataset) {
      it(name, function() {
        expect(converter['convertSimpleCondition'](equalsOperatorDataset[name].input)).toEqual(
          equalsOperatorDataset[name].expected
        )
      })
    }

    function make_the_other_operators_dataset(name: string, operator: string, alternate: string, mgOpt: string) {
      return {
        [name + ' case #1']: {
    ...
    function test_for_operator(operatorName: string, operator: string, alternate: string, mgOpt: string) {
      const dataset = make_the_other_operators_dataset(operatorName, operator, alternate, mgOpt)
      const converter = new MongodbConditionConverter([])
      for (const name in dataset) {
        it(name, function() {
          expect(converter['convertSimpleCondition'](<any>dataset[name].input)).toEqual(dataset[name].expected)
        })
      }
    }
