...
describe('DataConditionMatcher', function() {
    ...
    for (const data of dataset) {
      if (data.calls === false) {
        it('returns false if the operator is not match', function() {
          const matcher = new DataConditionMatcher('test', data.operator, 'compared', reader)
          const record: any = {
            getAttribute() {
              return 'record-value'
            }
      ...
      it('returns false if the operator is not match', function() {
        ...
        const matcher = new DataConditionMatcher('test', data.operator, 'compared', reader)
        const stub = Sinon.stub(matcher, data.calls as any)
        stub.returns(true)
  ...
  describe('.isEqual()', function() {
    ...
    for (let i = 0; i < dataset.length; i++) {
      ...
      it(description, function() {
        const matcher = new DataConditionMatcher('test', '=', dataset[i].b, reader)
        const record: any = {
          test: dataset[i].a
        }
  ...
  describe('.isInArray()', function() {
    ...
    it('calls Lodash.includes() to compare values', function() {
      ...
      const matcher = new DataConditionMatcher('test', '=', 'array', reader)
      const record: any = {
        test: 'record-value'
      }
