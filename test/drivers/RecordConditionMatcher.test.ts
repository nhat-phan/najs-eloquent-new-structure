...
describe('RecordConditionMatcher', function() {
    ...
    for (const data of dataset) {
      if (data.calls === false) {
        it('returns false if the operator is not match', function() {
          const matcher = new RecordConditionMatcher('test', data.operator, 'compared')
          const record: any = {
            getAttribute() {
              return 'record-value'
            }
  ...
  describe('.isEqual()', function() {
    ...
    for (let i = 0; i < dataset.length; i++) {
      ...
      it(description, function() {
        const matcher = new RecordConditionMatcher('test', '=', dataset[i].b)
        const record: any = {
          getAttribute() {
            return dataset[i].a
          }
  ...
  describe('.isInArray()', function() {
    ...
    it('calls Lodash.includes() to compare values', function() {
      ...
      const matcher = new RecordConditionMatcher('test', '=', 'array')
      const record: any = {
        getAttribute() {
          return 'record-value'
        }
