...
describe('MongodbConditionMatcher', function() {
  describe('.isMatch()', function() {
    it('always throws an exception', function() {
      const matcher = new MongodbConditionMatcher('test', '=', 'value')
      try {
        matcher.isMatch({})
      } catch (error) {
        expect(error.message).toEqual(
          'This class builds a condition for native matcher, please do not use isMatch() function.'
        )
        return
      }
  ...
  describe('.buildNativeCondition()', function() {
    ...
    for (const name in equalsOperatorDataset) {
      it(name, function() {
        ...
        const matcher = new MongodbConditionMatcher(input.field, input.operator, input.value)
        expect(matcher.getCondition()).toEqual(equalsOperatorDataset[name].expected)
      })
    }

    function extend_dataset(name: string, operator: string, alternate: string, mgOpt: string) {
      return {
        [name + ' case #1']: {
    ...
    function test_for_operator(operatorName: string, operator: string, alternate: string, mgOpt: string) {
      ...
      for (const name in dataset) {
        it(name, function() {
          ...
          const matcher = new MongodbConditionMatcher(input.field, input.operator as any, input.value)
